...
{
    class OrderPreservingPipeliningSpoolingTask<TOutput> : SpoolingTaskBase
    {
        ...
        private readonly QueryOperatorEnumerator<TOutput, int> m_partition; // The source partition.
        ...
        private readonly Queue<Pair<int, TOutput>>[] m_buffers; // The buffer for the results
        ...
        internal OrderPreservingPipeliningSpoolingTask(
            QueryOperatorEnumerator<TOutput, int> partition, 
            QueryTaskGroupState taskGroupState,
            bool[] consumerWaiting,
            bool[] producerWaiting,
            bool[] producerDone,
            int partitionIndex,
            Queue<Pair<int, TOutput>>[] buffers,
            object bufferLock,
        ...
        protected override void SpoolingWork()
        {
            ...
            Pair<int,TOutput>[] chunk = new Pair<int,TOutput>[chunkSize];
            ...
            do
            {
                ...
                {
                    chunk[lastChunkSize] = new Pair<int, TOutput>(key, element);
                    lastChunkSize++;
                }
                ...
                lock (m_bufferLock)
                {
                    ...
                    if (m_buffers[m_partitionIndex].Count >= OrderPreservingPipeliningMergeHelper<TOutput>.MAX_BUFFER_SIZE)
                    {
        ...
        public static void Spool(
            QueryTaskGroupState groupState, PartitionedStream<TOutput, int> partitions,
            bool[] consumerWaiting, bool[] producerWaiting, bool[] producerDone, 
            Queue<Pair<int,TOutput>>[] buffers, object[] bufferLocks,
            TaskScheduler taskScheduler, bool autoBuffered)
        {
            ...
            {
                buffers[i] = new Queue<Pair<int, TOutput>>(OrderPreservingPipeliningMergeHelper<TOutput>.INITIAL_BUFFER_SIZE);
            ...
            Task rootTask = new Task(
                () =>
                {
                    for (int i = 0; i < degreeOfParallelism; i++)
                    {
                        QueryTask asyncTask = new OrderPreservingPipeliningSpoolingTask<TOutput>(
