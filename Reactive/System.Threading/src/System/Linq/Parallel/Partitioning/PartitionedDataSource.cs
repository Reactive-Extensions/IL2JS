...
namespace System.Linq.Parallel
{
    ...
    internal class PartitionedDataSource<T> : PartitionedStream<T, int>
    {
        ...
        //     contain synchronization (i.e. the latter case), meaning callers may occassionally
        ...
        private void InitializePartitions(IEnumerable<T> source, int partitionCount, bool useStriping)
        {
            ...
            if (sourceAsList != null)
            {
                ...
                if (useStriping)
                {
                    ...
                    // @TODO: @PERF: @BUG#517: though we choose a chunk size that is a multiple of a cache line,
                    //     the CLR makes no guarantees about alignment, so in truth the actual elements
                    //     could overlap cache lines. We could try to align, but sadly the CLR's GC
                    //     gives no guarantees that things will remain aligned. We should look into
                    //     this in the future, possibly talking to the CLR team about a new feature.
        ...
        private class ContiguousChunkLazyEnumerator : QueryOperatorEnumerator<T, int>
        {
            ...
            internal override bool MoveNext(ref T currentElement, ref int currentKey)
            {
                ...
                while (true)
                {
                    ...
                    // @TODO: @BUG#510: revisit the use of Monitors as a synchronization primitive.
