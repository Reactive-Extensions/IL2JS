...
namespace System.Linq.Parallel
{
    ...
    internal class MergeExecutor<TInputOutput> : IEnumerable<TInputOutput>
    {
            ...
            if (isOrdered && !ignoreOutput)
            {
                if (options != ParallelMergeOptions.FullyBuffered && !partitions.OrdinalIndexState.IsWorseThan(OrdinalIndexState.Increasing))
                {
                    ...
                    if (partitions.PartitionCount > 1)
                    {
                        ...
                        mergeExecutor.m_mergeHelper = new OrderPreservingPipeliningMergeHelper<TInputOutput>(
                            (PartitionedStream<TInputOutput, int>)(object)partitions, taskScheduler, cancellationState, autoBuffered, queryId);
                    }
                    else
                    {
        ...
        internal static AsynchronousChannel<TInputOutput>[] MakeAsynchronousChannels(int partitionCount, ParallelMergeOptions options, CancellationToken cancellationToken)
        {
            ...
            {
                channels[i] = new AsynchronousChannel<TInputOutput>(chunkSize, cancellationToken);
            }
