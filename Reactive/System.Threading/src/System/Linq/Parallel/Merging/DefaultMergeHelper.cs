...
namespace System.Linq.Parallel
{
    ...
    internal class DefaultMergeHelper<TInputOutput, TIgnoreKey> : IMergeHelper<TInputOutput>
    {
            ...
            TaskScheduler taskScheduler, CancellationState cancellationState, int queryId)
        {
            ...
            if (!ignoreOutput)
            {
                ...
                if (options != ParallelMergeOptions.FullyBuffered)
                {
                    ...
                    {
                        m_asyncChannels =
                            MergeExecutor<TInputOutput>.MakeAsynchronousChannels(partitions.PartitionCount, options, cancellationState.MergedCancellationToken);
        ...
        //
        // @BUG#1546: There isn't much reason to call this method on a DefaultMergeHelper,
        // because DefaultMergeHelper does not have an array to efficiently hand out, and
        // has to build one up. Because of bug 1546, this method gets called under some
        // circumstances.
        //
