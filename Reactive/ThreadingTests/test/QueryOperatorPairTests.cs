...
{
    internal static class QueryOperatorPairTests
    {
        private delegate ParallelQuery<int> LeftOperatorFactory(bool orderPreserved);

        private class LeftOperator
        {
            private LeftOperatorFactory m_leftOperatorFactory;
            private bool m_orderWellDefined;
            ...
            public LeftOperatorFactory LeftOperatorFactory {
                get { return m_leftOperatorFactory; }
            }

            public bool OrderDefined
            {
                get { return m_orderWellDefined; }
            }

            public string OperatorName
            {
                get { return m_leftOperatorFactory.Method.Name.Substring(4); }
            }
        }
        ...
        {
            TestHarness.TestLog("   Running {0}/{1}, Order Preserved={2}", leftOpName, rightOpName, orderPreserved);
        }
        ...
        private class TestTracker
        {
            ...
            {
                m_passed = false;
                TestHarness.TestLog("   >> Failed. Expect:{0}  Got:{1}", a, b);
            }
            ...
            internal void MustSequenceEqual(IEnumerable<int> a, IEnumerable<int> b, bool orderPreserved)
            {
                ...
                {
                    m_passed = false;
                    TestHarness.TestLog("   >> Failed. Sequence 1: {0}", EnumerableToString(aa));
                    TestHarness.TestLog("   >> Failed. Sequence 2: {0}", EnumerableToString(ba));
                }
            }
