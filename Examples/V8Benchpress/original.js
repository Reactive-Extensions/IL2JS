...
// M a i n
// -----------------------------------------------------------------------------
var benchmarks = [ Fibonacci, Loop, Towers, Sieve, Permute, Queens, Recurse,
                   Sum, BubbleSort, QuickSort, TreeSort, Tak, Takl ];
...
function Main() {
    ...
    for (var i = 0; i < benchmarks.length; i++) {
        var testthen = new Date();
        for (var iter = 0; iter < 500; iter++)
            benchmarks[i].run();
        var testnow = new Date();
        Log(benchmarks[i].string + ": " + (testnow - testthen) + "ms");
    }
    var now = new Date();
    Log("Overall: " + (now - then) + "ms");
}

function Attach(inst, eventName, handler) {
    if (inst.addEventListener != null)
        inst.addEventListener(eventName, handler, false);
    else
        inst.attachEvent("on" + eventName, handler);
}

var button = document.createElement("BUTTON");
button.innerText = "Start";
Attach(button, "click", Main);
